...
def train(epoch, model, dataloader, criterion, optimizer, **kwargs):
    ...
    with tqdm(enumerate(dataloader), total=len(dataloader), desc=f"Train ({epoch:2d})", ncols=120) as t:
        for i, (images, labels) in t:
            ...
            ce = criterion(logits_ori, labels).mean()
    ...
    return Loss.item(), CE.item(), KL.item(), Acc.item()
...
def main(args):
    setup_seed(42)
    # net = model_builder.build("wideresnet-34-16", args.num_classes)
    ...
    print(f"Train loader batches: {len(train_loader)}, Valid loader batches: {len(valid_loader)}, Test loader batches: {len(test_loader)}")
    ...
    for epoch in range(1, args.epochs+1):
        loss, ce, kl, acc = train(epoch, net, train_loader, criterion, optimizer)
        ...
        recorder.update([epoch, lr, round(loss, 4), round(ce, 4), round(kl, 4), round(acc, 4), round(clean, 4), round(pgd10, 4)])
...
if __name__ == "__main__":
    """
    $ CUDA_VISIBLE_DEVICES=0 python AML/TRADES.py --epochs 150 --milestones 50 100 --suffix rn18/TRADES --debug
    """
    import argparse
